      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input
            ...
            className="mt-2 w-full accent-hapas-600"
          />
        ...
        <div>
          <label className="text-sm font-bold">⭐ Reviewfunnel</label>
          <div className="mt-1 flex gap-2">
            <button
          ...
          <p className="mt-1 text-xs text-stone-500">
            Compliant: de Google-optie blijft bij elke score zichtbaar (conform
        ...
        <div>
          <label className="text-sm font-bold">🔗 Google-review-link</label>
          <input
            value={draft.googleReviewUrl}
            onChange={(e) =>
              setDraft({ ...draft, googleReviewUrl: e.target.value })
            }
            className="mt-1 w-full rounded-xl border border-stone-200 p-2.5 text-sm"
            placeholder="https://search.google.com/local/writereview?placeid=..."
          />
        </div>

        <div>
          <label className="text-sm font-bold">🔐 Pincode personeel</label>
          <input
            ...
            className="mt-1 w-32 rounded-xl border border-stone-200 p-2.5 text-sm tracking-widest"
          />
        </div>
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl p-4">
      <header className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-black">📊 Manager — Hapas Noordwijk</h1>
        <Badge tone="hapas">{store.mode === "demo" ? "demo-modus" : "live"}</Badge>
      </header>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        />
        <Stat
          label="Gem. besteding / tafel"
          value={euro(stats?.avgSpendPerSessionCents ?? 0)}
          sub={`${stats?.avgOrdersPerSession ?? 0} bestellingen per tafel`}
        />
        <Stat
        ...
        <Stat
          ...
          sub="gem. van gesloten tafels"
        />
        <Stat
          label="Open keukentickets"
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          <SectionTitle>🏆 Populairste gerechten (vandaag)</SectionTitle>
          <Card>
            {stats && stats.popularProducts.length > 0 ? (
              <ol className="flex flex-col gap-1.5">
                {stats.popularProducts.map((p, i) => (
                  <li key={p.name} className="flex justify-between text-sm">
                    <span>
                      <span className="mr-2 font-black text-hapas-600">
                        {i + 1}.
                      </span>
                      {p.name}
                    </span>
              ...
              <p className="text-sm text-stone-500">Nog geen bestellingen vandaag.</p>
            )}
          </Card>
            ...
            {(reviews ?? []).length === 0 ? (
              ...
              (reviews ?? []).map((r) => (
                <Card key={r.id}>
                  ...
                  {r.feedback && (
                    <p className="mt-1 text-sm text-stone-700">“{r.feedback}”</p>
                  )}
                  {r.contact && (
                    <p className="mt-1 text-xs font-semibold text-hapas-700">
                      Contact: {r.contact}
                    </p>
                  )}
                </Card>
